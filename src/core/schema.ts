import type { SchemaDefinition, SchemaArtifact } from "./types.js";

const SPEC_DRIVEN_SCHEMA: SchemaDefinition = {
  name: "spec-driven",
  version: 1,
...
const BUILTIN_SCHEMAS: Record<string, SchemaDefinition> = {
...
export function getAvailableSchemas(): SchemaDefinition[] {
  return Object.values(BUILTIN_SCHEMAS);
}

export function getArtifact(schema: SchemaDefinition, artifactId: string): SchemaArtifact | undefined {
  return schema.artifacts.find((a) => a.id === artifactId);
}

export function getArtifactTemplate(templateFile: string): string {
  return ARTIFACT_TEMPLATES[templateFile] ?? `<!-- Template not found: ${templateFile} -->`;
}

export function getApplyInstruction(schema: SchemaDefinition): string {
  return schema.apply.instruction;
}

export function getApplyRequires(schema: SchemaDefinition): string[] {
  return schema.apply.requires;
}

export function getDefaultSchema(): SchemaDefinition {

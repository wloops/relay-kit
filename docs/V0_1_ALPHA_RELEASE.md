...
- `name`: `ai-advisor-kit`，符合包名和 CLI 定位。
- `version`: `0.1.0`，符合 alpha 试用阶段。
- `description`: `Skills-first, CLI-assisted AI programming advisor workflow toolkit.`，准确描述工具定位。
- `bin.advisor`: `dist/cli.js`，符合 npm bin 入口和构建产物路径。
...
- `npm pack`: 通过，生成 `ai-advisor-kit-0.1.0.tgz`，tarball 共 32 个文件。
...
- `npm link`: 通过。
- `advisor --help`: 通过，能展示 `init`、`start`、`ask`、`resume`、`review`、`doctor`、`sync` 命令。
- simple 临时项目流程：通过，验证了 `advisor init --mode simple`、`advisor start`、`advisor ask`、`advisor review`、`advisor doctor`，并确认生成项目级 Skills 与 handoff 文件。
- OpenSpec 临时项目流程：通过，验证了 `advisor init --mode openspec`、`advisor start --change add-demo`、`advisor ask`、`advisor review`、`advisor doctor`，并确认 `EXECUTOR_TASK.md` 包含 `OpenSpec Change: add-demo`。

验证后已清理临时项目、全局 npm link 和本地 tarball。

## 边界确认

本次发布准备不新增 lane 并行功能，不新增 GUI，不自动调用模型 API，不改变 `advisor init/start/ask/resume/review/doctor/sync --skills` 主流程设计，也不发布正式 1.0。

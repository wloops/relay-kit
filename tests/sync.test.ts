...
async function tempProject(prefix = "relay-sync-"): Promise<string> {
...
async function writeSkill(root: string, skill: string, content = "# Skill\n"): Promise<void> {
  const dir = path.join(root, ".relay", "skills", skill);
...
test("sync --skills uses configured project targets by default", async () => {
  ...
  assert.ok(await exists(path.join(root, ".relay", "skills-sync.json")));
});

test("sync --skills honors explicit project targets", async () => {
  const root = await tempProject();
  await runInit(root, { mode: "simple" });
  await writeSkill(root, "relay-extra", "# Extra\n");

  await runSync(root, { skills: true, target: "claude" });

  assert.ok(await exists(path.join(root, ".claude", "skills", "relay-extra", "SKILL.md")));
  assert.equal(await exists(path.join(root, ".agents", "skills", "relay-extra", "SKILL.md")), false);

  await writeSkill(root, "relay-extra-all", "# Extra all\n");
  await runSync(root, { skills: true, target: "all" });

  assert.ok(await exists(path.join(root, ".claude", "skills", "relay-extra-all", "SKILL.md")));
  assert.ok(await exists(path.join(root, ".agents", "skills", "relay-extra-all", "SKILL.md")));
  assert.equal(await exists(path.join(root, ".codex", "skills", "relay-extra-all", "SKILL.md")), false);
});

test("sync --skills writes user targets only when user scope is explicit", async () => {
  const root = await tempProject();
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "relay-sync-home-"));
  await runInit(root, { mode: "simple" });
  await writeSkill(root, "relay-user", "# User skill\n");
  ...
  assert.ok(await exists(path.join(homeDir, ".claude", "skills", "relay-user", "SKILL.md")));
  assert.ok(await exists(path.join(homeDir, ".agents", "skills", "relay-user", "SKILL.md")));
...
test("sync --skills --dry-run reports work without writing files or manifest", async () => {
  const root = await tempProject();
  await runInit(root, { mode: "simple" });
  await writeSkill(root, "relay-dry-run", "# Dry run\n");

  const result = await runSync(root, { skills: true, target: "codex", dryRun: true });

  assert.match(result.summary, /dry-run: yes/);
  assert.match(result.summary, /create codex\/project relay-dry-run\/SKILL.md/);
  assert.equal(await exists(path.join(root, ".agents", "skills", "relay-dry-run", "SKILL.md")), false);
  assert.equal(await exists(path.join(root, ".relay", "skills-sync.json")), false);
});
...
test("sync --skills protects user-modified targets and --force overwrites after review", async () => {
  ...
  const source = path.join(root, ".relay", "skills", "relay-planner", "SKILL.md");
  const target = path.join(root, ".claude", "skills", "relay-planner", "SKILL.md");
  ...
  assert.match(forced.summary, /update claude\/project relay-planner\/SKILL.md/);
...
test("sync validates supported flags and source availability", async () => {
  ...
  await assert.rejects(() => runSync(root, { skills: true }), /Missing \.relay\/config\.json/);
